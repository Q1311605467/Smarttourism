...
<%@ page import="pojo.Scenery_Reservation" %>
<%@ page import="pojo.Scenery" %>
...
<html>
<head>
    <title>Scenery_Reservation</title>
    <meta charset="UTF-8">
    ...
    <script type="text/javascript">
        function confirmDel(scen_reserv_id)
        {
            if(window.confirm("确定要取消预约吗？Really？")){
                document.location="${pageContext.request.contextPath}/sys/user/usercancelSR.html?user_id=${userSession.user_id}&scen_reserv_id="+scen_reserv_id
            }
        }
    </script>
</head>
...
<div class="book-main">
    ...
    <div class="right-module">
        <c:if test="${empty SRMyList}">
            <font color="red"><p>暂无订单哦~</p></font>
        </c:if>
        <c:forEach items="${SRMyList}" var="SRList">
            <div class="HR-My-List">
                <div class="HR-My-List-item1">
                    <div>
                        <h3>${SRList.scenery_name}</h3>
                        <p class="detail" >
<%--                        <span style="margin-right: 20px">--%>
<%--                                ${HRList.room_type}--%>
<%--                        </span>--%>
                            <span style="margin-right: 20px">
                                    预约张数：${SRList.reserv_num}
                            </span>
                        </p>
                        <p class="detail" >
                            <c:set var="ccid" value="${SRList}" scope="session" /><!-- 把该条评论上传到session中 -->
                            <%
                                Scenery_Reservation uom=(Scenery_Reservation)request.getSession().getAttribute("ccid");
                                SimpleDateFormat sdfas = new SimpleDateFormat("yyyy-MM-dd");//设定时间的范围到 分钟
                                String intime = sdfas.format(uom.getDate());//将获取到的时间转变为设定好的类型
                            %>
                            <span style="margin-right: 20px">预订时间：<%=intime%></span>
                        </p>
                        <p class="detail">合计：${SRList.total_cost_s}</p>
                        <c:if test="${SRList.is_cancel==0}">
                            <button class="btn btn-default" type="submit" style="margin-left: 70%;"
                                    onclick="confirmDel(${SRList.scen_reserv_id})">取消预约</button>
                        </c:if>
                        <c:if test="${SRList.is_cancel==1}">
    ...
    <div class="left-module">
        <div class="hotel-HR-left">
            ...
            <ul class="list-group">
                <li class="list-group-item-user1" style="padding-bottom: 20px;background-color: #6f88cc"><a href="${pageContext.request.contextPath }/sys/user/SRMylist/${userSession.user_id }">门票预订</a> </li>
                <li class="list-group-item-user1"><a href="${pageContext.request.contextPath }/sys/user/HRMylist/${userSession.user_id }">酒店预订</a></li>
            </ul>
